...
# Global variables to store the previous network and node statuses
previous_network_status = {"network_one": None, "network_two": None}
previous_node_status = {}

# Function to read network.csv
...
# Function to periodically check the network status
def check_network_status(update, context):
    try:
        # Read the network.csv file
        network_df = pd.read_csv('network.csv')
        ...
        # Check if the first value is zero
        if network_df['Value1'][0] == 0:
            if previous_network_status["network_one"] != 0:
                send_status_update("Network One is down.")
            previous_network_status["network_one"] = 0
        else:
            previous_network_status["network_one"] = 1

        # Check if the second value is zero
        if network_df['Value2'][0] == 0:
            if previous_network_status["network_two"] != 0:
                send_status_update("Network Two is down.")
            previous_network_status["network_two"] = 0
        else:
            previous_network_status["network_two"] = 1
    except Exception as e:
        print(f"An error occurred while checking network status: {str(e)}")

# Function to periodically check the node status
def check_node_status(update, context):
    try:
        # Read the node.csv file
        node_df = pd.read_csv('node.csv')
...
def main():
    ...
    # Schedule the periodic network and node status checks
